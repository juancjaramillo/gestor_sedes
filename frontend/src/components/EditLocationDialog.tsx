...
function extractApiErrorMessage(e: unknown): string {
  if (typeof e === "object" && e !== null) {
    ...
    const data = (resp?.data as Record<string, unknown>) ?? (anyErr.data as Record<string, unknown>) ?? {};
    const nested =
      (data.errors as Record<string, string[] | undefined>) ??
      ((data.error as Record<string, unknown>)?.["details"] as Record<string, string[] | undefined> | undefined);
    const pick = (k: string) => (nested && Array.isArray(nested[k]) ? nested[k]![0] : undefined);
    return (
      pick("image") ||
      pick("code") ||
      pick("name") ||
...
export default function EditLocationDialog({ open, onClose, item, onUpdated }: Props) {
      ...
      <form onSubmit={onSubmit}>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {preview ? (
              <img src={preview} alt="preview" style={{ width: 220, height: 140, objectFit: "cover", borderRadius: 8 }} />
            ) : item.image ? (
              <img src={item.image} alt={item.name} style={{ width: 220, height: 140, objectFit: "cover", borderRadius: 8 }} />
            ) : null}
          </Stack>
        </DialogContent>
        <DialogActions>
